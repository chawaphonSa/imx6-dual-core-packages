...
public class NativeNfcTag implements TagEndpoint {
    ...
    private int mIsPresent; // Whether the tag is known to be still present
    ...
    public synchronized void startPresenceChecking() {
        // todo: update this to real value here?
        mIsPresent = 1;
    }
    ...
    public synchronized byte[] readNdef() {
        ...
        try{
			...
			// native code will set lastError to get retrieved from framework code
			// lastError =  ErrorCodes.ERROR_IO;
			Log.d(TAG, "!Error: native doRead Failed");
		}
        return result;
    }
    ...
    public synchronized boolean isNdefFormatable() {
       if (hasTech(TagTechnology.NFC_B))
       {
         // None of these types can be formated, so we can 
         // directly stop here.
         return false;
       }
       
       // all other tag-types *may* be formattable. The NFCStack will decide this.
       return true;
    }
                ...
                switch (mTechList[i]) {
                    ...
                    case TagTechnology.ISO_DEP: {
                        try {
                          ...
                          Log.d(TAG, "Error: tag.mTechActBytes["+ i + "] was null for Tech ISO_DEP");
                        }
                        break;
                    }
